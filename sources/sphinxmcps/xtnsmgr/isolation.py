...
def import_builtin_processor(
    package_name: str, processor_name: str
) -> types.ModuleType:
    ''' Import a builtin processor module by constructing its path. '''
    module_path = f"{package_name}.processors.{processor_name}"
    try:
        _scribe.debug( f"Importing builtin processor: {module_path}" )
        module = importlib.import_module( module_path )
    except ImportError as exc:
        _scribe.error( f"Failed to import builtin {module_path}: {exc}" )
        raise
    else:
        _scribe.info( f"Successfully imported builtin: {module_path}" )
        return module


def import_external_processor( module_name: str ) -> types.ModuleType:
    ''' Import an external processor module by name.
    
        Uses standard Python import machinery. Version conflicts will
        surface naturally if they occur.
    '''
    try:
        _scribe.debug( f"Importing external processor: {module_name}" )
        module = importlib.import_module( module_name )
    except ImportError as exc:
        _scribe.error( f"Failed to import external {module_name}: {exc}" )
        raise
    else:
        _scribe.info( f"Successfully imported external: {module_name}" )
        return module
...
def reload_processor_module( module_name: str ) -> types.ModuleType:
    ...
    return import_external_processor( module_name )


def list_imported_processors( ) -> list[ str ]:
    ''' List all currently imported processor modules. '''
    # Look for modules that might be processors
    # This is heuristic-based since we don't have a formal registry
    return [
        module_name for module_name in __.sys.modules
        if ( 'processor' in module_name.lower( ) or
             'extension' in module_name.lower( ) )
    ]
